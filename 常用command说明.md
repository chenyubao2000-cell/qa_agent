...
**关键认知**
- Agent 不能"自启动"（不会定时跑、不会被外部事件触发），但**有 3 种调用路径**：
  1. **Command 显式调起**：command 的 markdown 里写 `Agent(subagent_type=..., prompt=...)` —— 本项目流水线主要走这条
  2. **用户在主对话里用自然语言喊起**：例如"用 `Explore` agent 帮我找下 XX 在哪定义"、"启动一个 `e2e-orchestrator` 跑下这个 PRD"，主 Claude 看懂后会调起对应 agent
  3. **主 Claude 自动路由 (auto-delegation)**：当你的提问描述自然匹配某个 agent 的 `description` 字段时，主 Claude 会自动选 agent 帮你跑——能不能命中取决于 agent 的 description 写得准不准
...
| **谁触发它** | 用户在命令行输入 `/xxx` | ① command 显式 `Agent(subagent_type=...)`<br>② 用户在主对话用自然语言喊起<br>③ 主 Claude 按 description 自动路由 | Agent / Command 主动 `Read` 它 |
