...
/**
 * POM fragment: People Data download area (§ 3.2.5)
...
export class TaskPagePeopleDataDownloadFragment {
  ...
  constructor(page: Page, i18n?: I18n) {
    ...
    this.peopleDataCard = page.locator('[role="log"] div[role="button"]').filter({
    ...
    this.peopleDataPanelDownloadBtn = i18n
      ? page.locator(`div.flex.h-12 button[title="${i18n.t('canvas.downloadFile')}"]`).first()
      : page.locator('div.flex.h-12 button[title]').first();

    // Close button in People Data panel header (title attr from i18n)
    this.peopleDataPanelCloseBtn = i18n
      ? page.locator(`div.flex.h-12 button[title="${i18n.t('canvas.close')}"]`).first()
      : page.locator('div.flex.h-12 button[title]').last();
  ...
  getPeopleDataPanelDownloadButton(): Locator {
    ...
    const label = this.i18n ? this.i18n.t('canvas.downloadFile') : null;
    return label
      ? panelHeader.locator(`button[title="${label}"]`).first()
      : panelHeader.locator('button[title]').first();
  }
  ...
  getPeopleDataPanelCloseButton(): Locator {
    const panelHeader = this.page.locator('div.flex.h-12');
    const label = this.i18n ? this.i18n.t('canvas.close') : null;
    return label
      ? panelHeader.locator(`button[title="${label}"]`).first()
      : panelHeader.locator('button[title]').last();
  }
  ...
  async gotoTask(taskUrl: string): Promise<void> {
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        await this.page.locator('[role="log"]').waitFor({ state: 'visible', timeout: 15_000 }).catch(() => {});
        return;
      } catch (e) {
  ...
  async openPeopleDataPanel(): Promise<void> {
    await this.page.locator('[role="log"]').waitFor({ state: 'visible', timeout: 15_000 });
