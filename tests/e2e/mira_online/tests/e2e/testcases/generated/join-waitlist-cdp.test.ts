...
test.describe("[CDP] Join Waitlist Form", () => {
  ...
  test(
    ...
    async ({ page, i18n }) => {
      ...
      await expect(
        joinPage.getToastByText(
          i18nRegex("auth.joinWaitlist.errors.emailNotVerified"),
        ),
      ).toBeVisible();
    },
  );
  ...
  test(
    ...
    async ({ page, i18n }) => {
      ...
      await expect(
        joinPage.getToastByText(i18nRegex("auth.validation.emailInvalid")),
      ).toBeVisible();
    },
  );
  ...
  test(
    "TC-CDP-JW-005 填写邮箱后发送验证码按钮从禁用变为启用",
    { tag: ["@P1", "@regression", "@full"] },
    async ({ page, i18n }) => {
      const joinPage = new JoinWaitlistPage(page, i18n);
      await joinPage.goto();
  ...
  test(
    ...
    async ({ page, i18n }) => {
      ...
      await expect(
        joinPage.getToastByText(i18nRegex("auth.joinWaitlist.otpSent")),
      ).toBeVisible();
    },
  );
  ...
  test(
    ...
    async ({ page, i18n }) => {
      ...
      await expect(joinPage.getHeading()).toHaveText(
      ...
      await expect(joinPage.getSubmitButton()).toHaveText(
        i18nRegex("auth.joinWaitlist.submit"),
      );
      await expect(joinPage.getCancelButton()).toBeVisible();
      await expect(joinPage.getCancelButton()).toHaveText(
        i18nRegex("auth.joinWaitlist.cancel"),
      );
    },
  );
