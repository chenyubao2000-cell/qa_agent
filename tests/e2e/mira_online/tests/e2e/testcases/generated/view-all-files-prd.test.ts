...
async function waitForTaskReady(
  ...
  // Avoid waitForLoadState("networkidle") — the app streams/polls in the
  // background so it never settles and burns the test timeout. Instead wait
  // for the chat log to become visible (same pattern as tool-chain-smoke).
  await page
    .locator('[role="log"]')
...
test.describe("US-VF-ENTRY - View All Files -- entry visibility", () => {
  ...
  test(
    ...
    async ({ page, i18n, taskWithToolChainUrl }) => {
      ...
      await expect(vaf.getViewAllFilesToolbarBtn()).toBeVisible({
        timeout: 30_000,
      });
    },
  );
  ...
  test(
    "TC-PRD-VF-007: result area button visible in chat with files",
    {
    ...
    async ({ page, i18n, taskWithToolChainUrl }) => {
      const vaf = new ViewAllFilesFragment(page, i18n);
      await waitForTaskReady(page, taskWithToolChainUrl);
      await expect(vaf.getViewAllFilesResultBtn()).toBeVisible({
        timeout: 15_000,
      });
    },
  );
...
test.describe("US-VF-MODAL - View All Files -- panel open and close", () => {
  ...
  test(
    ...
    async ({ page, i18n, taskWithToolChainUrl }) => {
      ...
      await waitForTaskReady(page, taskWithToolChainUrl);
      await vaf
        .getViewAllFilesToolbarBtn()
        .waitFor({ state: "visible", timeout: 30_000 });
      await vaf.clickViewAllFilesToolbar();
      await vaf.waitForFilesPanelOpen();
      await expect(vaf.getFilesPanelTitle()).toBeVisible();
      await expect(vaf.getFilesPanelTitle()).toContainText(
        i18nRegex("taskFiles.title"),
      );
    },
  );

  test(
    "TC-PRD-VF-004: clicking result area button opens files panel",
    {
      tag: ["@P1", "@regression", "@full"],
    },
    async ({ page, i18n, taskWithToolChainUrl }) => {
  ...
  test(
    "TC-PRD-VF-005: files panel shows file cards after opening",
    {
      tag: ["@P1", "@regression", "@full"],
    },
    async ({ page, i18n, taskWithToolChainUrl }) => {
  ...
  test(
    "TC-PRD-VF-014: close button dismisses files panel",
    {
      tag: ["@P1", "@regression", "@full"],
    },
    async ({ page, i18n, taskWithToolChainUrl }) => {
  ...
  test(
    "TC-PRD-VF-019: batch download icon button visible in panel header",
    {
      tag: ["@P1", "@regression", "@full"],
    },
    async ({ page, i18n, taskWithToolChainUrl }) => {
...
test.describe("US-VF-BATCH - View All Files -- batch download", () => {
  ...
  test(
    "TC-PRD-VF-008: clicking select all checks all file checkboxes",
    {
      tag: ["@P1", "@regression", "@full"],
    },
    async ({ page, i18n, taskWithToolChainUrl }) => {
  ...
  test(
    "TC-PRD-VF-009: selecting a file enables batch download button",
    {
      tag: ["@P1", "@regression", "@full"],
    },
    async ({ page, i18n, taskWithToolChainUrl }) => {
  ...
  test(
    "TC-PRD-VF-012: clicking cancel exits selection mode",
    {
      tag: ["@P1", "@regression", "@full"],
    },
    async ({ page, i18n, taskWithToolChainUrl }) => {
  ...
  test(
    "TC-PRD-VF-020: clicking select all again deselects all files",
    {
      tag: ["@P2", "@full"],
    },
    async ({ page, i18n, taskWithToolChainUrl }) => {
  ...
  test(
    "TC-PRD-VF-021: individual file download button is visible",
    {
      tag: ["@P2", "@full"],
    },
    async ({ page, i18n, taskWithToolChainUrl }) => {
      ...
      await vaf.waitForFilesPanelOpen();
      await vaf.waitForFileCardsLoaded();
      // In browse mode, each file card has a download button (title varies by locale)
      // Scope to the files panel inner root div (bg-background text-foreground flex flex-col)
      const panelScope = page.locator("div.bg-background.text-foreground.flex.flex-col.h-full.w-full").last();
      const downloadBtns = panelScope.locator(
        i18nTitleSelector("button", "taskFiles.download"),
      );
...
test.describe("US-VF-E2E - View All Files -- end-to-end scenarios", () => {
  ...
  test(
    "TC-PRD-VF-016: navigating away closes files panel",
    {
      tag: ["@P2", "@full"],
    },
    async ({ page, i18n, taskWithToolChainUrl }) => {
