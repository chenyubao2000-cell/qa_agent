...
async function createAuthenticatedPage(browser: import('@playwright/test').Browser): Promise<{ ctx: BrowserContext; page: Page }> {
...
test.describe('US-SIDEBAR-01 · 侧边栏可见性控制', () => {
  ...
  test(
    ...
    async ({ page, i18n }) => {
      ...
      await expect(taskPage.getWelcomeHeading()).toHaveText(/我能为你做什么|What can I do for you/);
...
test.describe('US-SIDEBAR-03 · 任务右键菜单', () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      await expect(taskPage.getContextMenuItemShare()).toHaveText(/分享|Share/);
      await expect(taskPage.getContextMenuItemRename()).toBeVisible();
      await expect(taskPage.getContextMenuItemRename()).toHaveText(/重命名|Rename/);
      await expect(taskPage.getContextMenuItemDelete()).toBeVisible();
      await expect(taskPage.getContextMenuItemDelete()).toHaveText(/删除|Delete/);
    }
  );
...
test.describe('US-SIDEBAR-04 · 分享弹窗', () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      await expect(taskPage.getCreateShareLinkBtn()).toHaveText(/创建分享链接|Create Share Link/);
...
test.describe.serial('US-SIDEBAR-05 · 重命名弹窗', () => {
  ...
  test(
    ...
    async ({ page, i18n }) => {
      ...
      await expect(taskPage.getRenameSaveBtn()).toBeDisabled({ timeout: 3000 }).catch(async () => {
...
test.describe.serial('US-SIDEBAR-06 · 删除弹窗', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    await p.waitForURL(/\/task\/.+/, { timeout: 60_000 });
    await taskPage.goto();
    deleteTaskName = await taskPage.getFirstTaskName();
    ...
    await p.waitForURL(/\/task\/.+/, { timeout: 60_000 });
    await taskPage.goto();
    cancelTaskName = await taskPage.getFirstTaskName();
  ...
  test(
    ...
    async ({ page, i18n }) => {
      ...
      await expect(taskPage.getDeleteDialogWarning()).toHaveText(/此操作无法撤销|cannot be undone/);
  ...
  test(
    ...
    async ({ page, i18n }) => {
      ...
      // Verify delete dialog closes (deletion confirmed)
      await expect(taskPage.getDeleteDialog()).toBeHidden();
  ...
  test.fixme(
    ...
    async ({ page, i18n }) => {
      ...
      await expect(taskPage.getTaskItemByName(actualName)).toHaveCount(0, { timeout: 10_000 });
    }
  );
  ...
  test.afterAll(async ({ browser }) => {
    ...
    try {
      ...
      await taskPage.clickDeleteMenuItem();
      await taskPage.confirmDelete();
    } catch { /* task already deleted or not found */ }
    await ctx.close();
  });
});
