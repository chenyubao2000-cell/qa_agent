...
test.describe('US-CONV-FILEUPLOAD · 文件上传功能', () => {
  ...
  test.skip('TC-CDP-CONV-001 附加支持格式文件后 Submit 按钮变为可点击', { tag: ['@P0'] }, async ({ page, i18n }) => {
  ...
  test.skip('TC-CDP-CONV-002 同时附加多个文件时均显示在附件区', { tag: ['@P1'] }, async ({ page, i18n }) => {
    const taskPage = new TaskPage(page, i18n);
    await taskPage.goto();
  ...
  test.skip('TC-CDP-CONV-005 附加单个文件后文件名和 Remove 按钮显示', { tag: ['@P1'] }, async ({ page, i18n }) => {
    const taskPage = new TaskPage(page, i18n);
    await taskPage.goto();
  ...
  test.skip('TC-CDP-CONV-007 点击 Remove attachment 后文件被移除且 Submit 重新禁用', { tag: ['@P1'] }, async ({ page, i18n }) => {
    const taskPage = new TaskPage(page, i18n);
    await taskPage.goto();
  ...
  // DEPRECATED: covered by upload-input-prd TC-PRD-UPINP-012
  test.skip('TC-CDP-CONV-010 文件附加与移除的完整状态流转（S0→S1→S0）', { tag: ['@P0'] }, async ({ page, i18n }) => {
    const taskPage = new TaskPage(page, i18n);
    await taskPage.goto();
...
test.describe('US-CONV-MSGSEND · 消息发送功能', () => {
  ...
  test('TC-CDP-CONV-003 仅上传文件（无文字）点击 Submit 后导航至任务详情', { tag: ['@P0', '@smoke', '@regression', '@full'] }, async ({ page, i18n }) => {
  ...
  test('TC-CDP-CONV-004 未附加任何内容时 Submit 按钮保持禁用', { tag: ['@P1', '@regression', '@full'] }, async ({ page, i18n }) => {
    const taskPage = new TaskPage(page, i18n);
    await taskPage.goto();
  ...
  test('TC-CDP-CONV-006 点击场景建议按钮自动提交并导航至任务详情', { tag: ['@P1', '@regression', '@full', '@failing'] }, async ({ page, i18n }) => {
    test.setTimeout(60_000);
    const taskPage = new TaskPage(page, i18n);
    await taskPage.goto();
  ...
  test('TC-CDP-CONV-008 发送消息后聊天日志显示 AI 标签和思考中状态', { tag: ['@P1', '@regression', '@full'] }, async ({ page, i18n }) => {
    test.setTimeout(60_000);
    const taskPage = new TaskPage(page, i18n);
    await taskPage.goto();
  ...
  test('TC-CDP-CONV-009 用户消息发送后显示在聊天日志中并带复制按钮', { tag: ['@P1', '@regression', '@full'] }, async ({ page, i18n }) => {
    test.setTimeout(60_000);
    const taskPage = new TaskPage(page, i18n);
    await taskPage.goto();
    ...
    await expect(page).toHaveURL(/\/task\/.+/, { timeout: 30_000 });
    await expect(taskPage.getChatLog()).toBeVisible();
    await expect(taskPage.getCopyButton()).toBeVisible();
    await expect(taskPage.getCopyButton()).toHaveText(/复制|Copy/);
  });

  test('TC-CDP-CONV-013 上传文件 + 输入文字 + 发送，验证聊天记录同时显示文件和文字', { tag: ['@P0', '@smoke', '@regression', '@full'] }, async ({ page, i18n }) => {
    test.setTimeout(60_000);
    const taskPage = new TaskPage(page, i18n);
    await taskPage.goto();
  ...
  test('TC-CDP-CONV-015 点击场景建议按钮直接触发任务创建', { tag: ['@P1', '@regression', '@full', '@failing'] }, async ({ page, i18n }) => {
    test.setTimeout(60_000);
    const taskPage = new TaskPage(page, i18n);
    await taskPage.goto();
...
test.describe('US-CONV-CHATRESPONSE · AI 响应结构', () => {
  ...
  test('TC-CDP-CONV-018 聊天日志区域有 role=log aria-live=polite 可访问性属性', { tag: ['@P1', '@regression', '@full'] }, async ({ page, i18n }) => {
    const taskPage = new TaskPage(page, i18n);
    await taskPage.goto();
  ...
  test('TC-CDP-CONV-017 工具卡片在进行中状态下显示有色边框', { tag: ['@P2', '@full'] }, async ({ page, i18n }) => {
    test.setTimeout(60_000);
    const taskPage = new TaskPage(page, i18n);
    await taskPage.goto();
    ...
    const appeared = await toolCards.first().waitFor({ state: 'visible', timeout: 30_000 }).then(() => true).catch(() => false);
    if (appeared) {
...
test.describe('US-CONV-WORKSPACE · 工作区面板', () => {
  ...
  test('TC-CDP-CONV-011 点击工具卡片打开工作区面板（S3→S5）', { tag: ['@P0', '@smoke', '@regression', '@full'] }, async ({ page, i18n }) => {
    ...
    await expect(taskPage.getWorkspacePanel()).toBeVisible({ timeout: 10_000 });
  ...
  test('TC-CDP-CONV-012 点击工作区面板关闭按钮后面板消失（S5→S3）', { tag: ['@P1', '@regression', '@full'] }, async ({ page, i18n }) => {
    ...
    await expect(taskPage.getWorkspacePanel()).toBeHidden({ timeout: 10_000 });
    await expect(taskPage.getChatLog()).toBeVisible();
  });
  ...
  test('TC-CDP-CONV-016 工作区面板打开后文件上传按钮仍可访问', { tag: ['@P1', '@regression', '@full'] }, async ({ page, i18n }) => {
    ...
    await expect(taskPage.getFileUploadButton()).toHaveAttribute('aria-label', /添加照片或文件|Add photos or files/);
...
test.describe('US-CONV-I18N · i18n 文本验证', () => {
  ...
  test('TC-CDP-CONV-019 添加照片或文件按钮 i18n 文本正确', { tag: ['@P2', '@full'] }, async ({ page, i18n }) => {
    const taskPage = new TaskPage(page, i18n);
    await taskPage.goto();

    // Button displays as "+" icon; verify it exists by accessible name (i18n)
    await expect(taskPage.getFileUploadButton()).toBeVisible();
