...
test.describe('[CDP] Sign-In Page', () => {
  ...
  test('TC-CDP-SIGNIN-002 输入无效邮箱格式后显示验证错误', { tag: ['@P1', '@regression', '@full'] }, async ({ page, i18n }) => {
    const signIn = new SignInPage(page, i18n);
    await signIn.goto();
    ...
    await expect(signIn.getEmailValidationError()).toHaveText(/Please enter a valid email address|请输入有效的邮箱地址/);
  });

  test('TC-CDP-SIGNIN-003 邮箱输入框为空时继续按钮禁用', { tag: ['@P1', '@regression', '@full'] }, async ({ page, i18n }) => {
    const signIn = new SignInPage(page, i18n);
    await signIn.goto();
  ...
  test('TC-CDP-SIGNIN-004 密码输入框为空时继续按钮禁用', { tag: ['@P1', '@regression', '@full', '@failing'] }, async ({ page, i18n }) => {
    const signIn = new SignInPage(page, i18n);
    await signIn.goto();
  ...
  test('TC-CDP-SIGNIN-006 输入错误密码后显示凭证错误信息', { tag: ['@P1', '@regression', '@full'] }, async ({ page, i18n }) => {
    ...
    await expect(signIn.getCredentialError()).toHaveText(/Invalid email or password\. Please check and try again|邮箱或密码错误，请检查后重试/);
  });

  test('TC-CDP-SIGNIN-007 密码步骤点击编辑返回邮箱步骤', { tag: ['@P1', '@regression', '@full', '@failing'] }, async ({ page, i18n }) => {
    const signIn = new SignInPage(page, i18n);
    await signIn.goto();
    await signIn.goToPasswordStep(TEST_EMAIL);
    ...
    await expect(signIn.getTitleHeading()).toHaveText(/Sign in or Sign up|登录或注册/);
  });

  test('TC-CDP-SIGNIN-008 密码步骤点击返回登录返回邮箱步骤', { tag: ['@P1', '@regression', '@full', '@failing'] }, async ({ page, i18n }) => {
    const signIn = new SignInPage(page, i18n);
    await signIn.goto();
    await signIn.goToPasswordStep(TEST_EMAIL);
  ...
  test('TC-CDP-SIGNIN-009 点击显示密码切换为明文显示', { tag: ['@P2', '@full'] }, async ({ page, i18n }) => {
    const signIn = new SignInPage(page, i18n);
    await signIn.goto();
    await signIn.goToPasswordStep(TEST_EMAIL);
  ...
  test('TC-CDP-SIGNIN-010 点击隐藏密码切换回密文显示', { tag: ['@P2', '@full'] }, async ({ page, i18n }) => {
    const signIn = new SignInPage(page, i18n);
    await signIn.goto();
    await signIn.goToPasswordStep(TEST_EMAIL);
    await signIn.fillPassword('Test@12345678');
  ...
  test('TC-CDP-SIGNIN-011 完整登录流程 — 邮箱 → 密码 → 成功跳转', { tag: ['@P0', '@smoke', '@regression', '@full'] }, async ({ page, i18n }) => {
    ...
    await expect(signIn.getMicrosoftOAuthButton()).toContainText(/Continue with Microsoft|使用 Microsoft 登录/);
  ...
  test('TC-CDP-SIGNIN-013 验证 Microsoft OAuth 按钮可见', { tag: ['@P1', '@regression', '@full', '@failing'] }, async ({ page, i18n }) => {
    const signIn = new SignInPage(page, i18n);
    await signIn.goto();
  ...
  test('TC-CDP-SIGNIN-014 密码步骤验证忘记密码链接可见', { tag: ['@P2', '@full'], annotation: { type: 'skip', description: 'Forgot password link only appears for registered accounts (Enter password step), not for Create password step' } }, async ({ page, i18n }) => {
    test.skip();
    const signIn = new SignInPage(page, i18n);
    await signIn.goto();
    await signIn.goToPasswordStep(TEST_EMAIL);
    ...
    await expect(signIn.getForgotPasswordLink()).toHaveText(/Forgot password|忘记密码/);
  });

  test('TC-CDP-SIGNIN-015 密码步骤邮箱显示为只读', { tag: ['@P2', '@full', '@failing'] }, async ({ page, i18n }) => {
    const signIn = new SignInPage(page, i18n);
    await signIn.goto();
    await signIn.goToPasswordStep(TEST_EMAIL);
