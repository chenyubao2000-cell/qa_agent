...
test.describe("US-TASK-INPUT · Chat Input Area", () => {
  test(
    "TC-CDP-TASK-015 新任务页面显示欢迎标题、输入框和场景建议",
  ...
  test(
    "TC-CDP-TASK-001 输入非空文本后 Submit 按钮启用",
  ...
  test(
    "TC-CDP-TASK-002 输入框为空时 Submit 按钮禁用",
    { tag: ["@P1", "@regression", "@full"] },
    async ({ page, i18n }) => {
      const taskPage = new TaskPage(page, i18n);
      await taskPage.goto();
  ...
  test(
    "TC-CDP-TASK-016 点击场景建议自动提交并导航至任务详情",
    { tag: ["@P1", "@regression", "@full"] },
    async ({ page, i18n }) => {
      const taskPage = new TaskPage(page, i18n);
      await taskPage.goto();
...
test.describe("US-TASK-SIDEBAR · Sidebar Navigation", () => {
  test(
    "TC-CDP-TASK-004 点击 Toggle Sidebar 折叠侧边栏",
    { tag: ["@P1", "@regression", "@full"] },
    async ({ page, i18n }) => {
      const taskPage = new TaskPage(page, i18n);
      await taskPage.goto();
  ...
  test(
    ...
    async ({ page, i18n }) => {
      ...
      const isVisible = await taskPage
        .getTasksSectionButton()
  ...
  test(
    'TC-CDP-TASK-006 点击"任务"区段标题折叠任务列表',
    { tag: ["@P1", "@regression", "@full"] },
    async ({ page, i18n }) => {
      const taskPage = new TaskPage(page, i18n);
      await taskPage.goto();
      ...
      if (initialCount > 0) {
        ...
        await expect(
          page.locator('[data-slot="collapsible"][data-state="closed"]'),
        ).toBeVisible();
      }
    },
  );
  ...
  test(
    ...
    async ({ page, i18n }) => {
      ...
      await taskPage.clickTasksSection();
      await expect(
        page.locator('[data-slot="collapsible"][data-state="open"]'),
      ).toBeVisible();
  ...
  test(
    "TC-CDP-TASK-008 点击任务 More 按钮打开上下文菜单",
    { tag: ["@P1", "@regression", "@full"] },
    async ({ page, i18n }) => {
      const taskPage = new TaskPage(page, i18n);
      await taskPage.goto();
  ...
  test(
    "TC-CDP-TASK-009 按 Escape 关闭上下文菜单",
    { tag: ["@P1", "@regression", "@full"] },
    async ({ page, i18n }) => {
      const taskPage = new TaskPage(page, i18n);
      await taskPage.goto();
  ...
  test(
    "TC-CDP-TASK-003 无任务时侧边栏任务列表区域为空",
    { tag: ["@P2", "@full"] },
    async ({ page, i18n }) => {
      const taskPage = new TaskPage(page, i18n);
      await taskPage.goto();
...
test.describe("US-TASK-USERMENU · User Menu", () => {
  test(
    "TC-CDP-TASK-010 点击用户菜单按钮打开用户菜单",
    { tag: ["@P1", "@regression", "@full"] },
    async ({ page, i18n }) => {
      const taskPage = new TaskPage(page, i18n);
      await taskPage.goto();
      ...
      await expect(taskPage.getUserMenuLanguageItem()).toHaveText(
        i18nRegex("Layout.language"),
      );
      await expect(taskPage.getUserMenuThemeItem()).toBeVisible();
      await expect(taskPage.getUserMenuThemeItem()).toHaveText(
        i18nRegex("Layout.theme"),
      );
      await expect(taskPage.getUserMenuSignOutItem()).toBeVisible();
      await expect(taskPage.getUserMenuSignOutItem()).toHaveText(
  ...
  test(
    ...
    async ({ page, i18n }) => {
      ...
      await expect(taskPage.getThemeOptionLight()).toHaveText(
        i18nRegex("Layout.light"),
      );
      await expect(taskPage.getThemeOptionDark()).toBeVisible();
      await expect(taskPage.getThemeOptionDark()).toHaveText(
        i18nRegex("Layout.dark"),
      );
      await expect(taskPage.getThemeOptionSystem()).toBeVisible();
      await expect(taskPage.getThemeOptionSystem()).toHaveText(
...
test.describe("US-TASK-DETAIL · Task Detail View", () => {
  test(
    "TC-CDP-TASK-013 点击侧边栏任务项进入任务详情页",
    { tag: ["@P0", "@smoke", "@regression", "@full"] },
    async ({ page, i18n }) => {
      const taskPage = new TaskPage(page, i18n);
      await taskPage.goto();
  ...
  test(
    'TC-CDP-TASK-014 在任务详情页点击"新建任务"返回空白视图',
    { tag: ["@P0", "@smoke", "@regression", "@full"] },
    async ({ page, i18n }) => {
      const taskPage = new TaskPage(page, i18n);
      await taskPage.goto();
