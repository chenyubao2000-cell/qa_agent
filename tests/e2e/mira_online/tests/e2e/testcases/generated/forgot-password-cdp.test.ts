...
test.describe("Forgot Password Form", () => {
  test(
    "TC-CDP-FP-001 输入有效邮箱后点击发送重置链接，进入确认步骤",
    ...
    async ({ page, i18n }) => {
      ...
      await expect(forgotPassword.getPageTitle()).toHaveText(
  ...
  test("TC-CDP-FP-002 输入无效邮箱后点击发送重置链接，显示验证错误", async ({
    page,
    i18n,
  }) => {
    const forgotPassword = new ForgotPasswordPage(page, i18n);
    ...
    await expect(forgotPassword.getValidationError()).toHaveText(
  ...
  test("TC-CDP-FP-006 用户完成完整的密码重置请求流程", async ({
    page,
    i18n,
  }) => {
    const forgotPassword = new ForgotPasswordPage(page, i18n);
  ...
  test("TC-CDP-FP-007 多次提交同一邮箱不导致页面崩溃", async ({
    page,
    i18n,
  }) => {
    const forgotPassword = new ForgotPasswordPage(page, i18n);
  ...
  test("TC-CDP-FP-008 页面初始状态正确显示所有 UI 元素", async ({
    page,
    i18n,
  }) => {
    const forgotPassword = new ForgotPasswordPage(page, i18n);
    ...
    await expect(forgotPassword.getPageDescription()).toContainText(
      i18nRegex("auth.forgotPasswordDescription"),
    );
    await expect(forgotPassword.getEmailInput()).toBeVisible();
    await expect(forgotPassword.getEmailInput()).toHaveAttribute(
      "type",
      "email",
    );
    ...
    await expect(forgotPassword.getBackToLoginLink()).toHaveAttribute(
      "href",
      /sign-in/,
    );
  });
...
test.describe("Forgot Password Navigation", () => {
  test('TC-CDP-FP-004 确认步骤点击"返回登录"返回邮箱输入步骤', async ({
    page,
    i18n,
  }) => {
    const forgotPassword = new ForgotPasswordPage(page, i18n);
  ...
  test('TC-CDP-FP-005 邮箱输入页点击"返回登录"链接导航到 /sign-in', async ({
    page,
    i18n,
  }) => {
    const forgotPassword = new ForgotPasswordPage(page, i18n);
