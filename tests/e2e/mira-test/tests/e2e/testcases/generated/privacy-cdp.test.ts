...
test.describe("[CDP] Privacy — load", { tag: ["@regression", "@full"] }, () => {
  test(
...
test.describe("[CDP] Privacy — content", { tag: ["@regression", "@full"] }, () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      const count = await privacyPage.sectionHeadings.count();
...
test.describe("[CDP] Privacy — footer", { tag: ["@regression", "@full"] }, () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      // Year is dynamic (new Date().getFullYear()) — never assert a hardcoded year.
...
test.describe("[CDP] Privacy — document metadata", { tag: ["@full"] }, () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      const title = await privacyPage.documentTitle();
      expect(title).toBe("Privacy Policy - Mira");
      const lang = await privacyPage.htmlLang();
      expect(lang).toBe("en");
    },
  );
});
