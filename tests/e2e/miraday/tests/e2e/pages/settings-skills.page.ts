...
export class SettingsSkillsPage {
  ...
  /**
   * Derive the first rendered official skill's name from its toggle switch's accessible name,
   * instead of hardcoding a specific skill (production's official skill set can differ from
   * other environments / change over time — see MIRA settings-skills prod adaptation notes).
   */
  async firstOfficialSkillName(): Promise<string> {
    const label = await this._skillSwitches.first().getAttribute("aria-label");
    const template = this.t(KEYS.toggleAriaLabel, "Enable or disable {name}");
    const [prefix] = template.split("{name}");
    return label && prefix && label.startsWith(prefix) ? label.slice(prefix.length) : (label ?? "");
  }
