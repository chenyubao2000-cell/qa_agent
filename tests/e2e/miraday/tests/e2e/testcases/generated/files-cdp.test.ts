...
test.describe("[CDP] Files — list load & toolbar", { tag: ["@regression", "@full"] }, () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      expect(await filesPage.fileRowCount()).toBeGreaterThan(0);
...
test.describe("[CDP] Files — search", { tag: ["@full"] }, () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      // No literal keyword supplied by the handoff (dataVariant: "no-match-keyword") —
      // any string absent from seeded file names satisfies the no-match scenario.
      await filesPage.search("zzzznonexistentfilename");
      await expect(filesPage.emptyTitle).toBeVisible();
