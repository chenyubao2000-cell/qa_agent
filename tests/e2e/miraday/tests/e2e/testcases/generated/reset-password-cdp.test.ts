...
test.describe("[CDP] Reset Password — form rendering", { tag: ["@regression", "@full"] }, () => {
  test(
...
test.describe("[CDP] Reset Password — password visibility", { tag: ["@full"] }, () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      expect(await resetPasswordPage.passwordInputType()).toBe("text");
      await expect(resetPasswordPage.passwordToggle).toHaveAttribute("aria-label", "Hide password");
    },
  );
});

test.describe("[CDP] Reset Password — validation", { tag: ["@regression", "@full"] }, () => {
  test(
