...
test.describe("[CDP] Forgot Password — page load", { tag: ["@regression", "@full"] }, () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      await expect(forgotPasswordPage.pageHeading).toHaveText("Forgot Password");
...
test.describe("[CDP] Forgot Password — email validation", { tag: ["@regression", "@full"] }, () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      await expect(forgotPasswordPage.emailError).toHaveText("Please enter a valid email address");
      expect(await forgotPasswordPage.emailAriaInvalid()).toBe("true");
    },
  );
  ...
  test(
    ...
    async ({ page, i18n }) => {
      ...
      expect(await forgotPasswordPage.emailAriaInvalid()).toBe("false");
      await expect(forgotPasswordPage.submitButton).toBeVisible();
    },
  );
});

test.describe("[CDP] Forgot Password — step transition", { tag: ["@regression", "@full"] }, () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      // Guard: continueButton must NEVER be clicked here — it triggers
      // authClient.requestPasswordReset and sends a real reset email.
