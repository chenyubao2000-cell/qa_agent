...
test.describe("[CDP] Library — page load", { tag: ["@regression", "@full"] }, () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      // URL stays /library (no redirect to /sign-in) + h1 renders "Library".
...
test.describe("[CDP] Library — sidebar shell", { tag: ["@regression", "@full"] }, () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      // Main content is unaffected by the sidebar collapse.
      await expect(libraryPage.heading).toBeVisible();
      await expect(libraryPage.canvasHeading).toBeVisible();
    },
  );
