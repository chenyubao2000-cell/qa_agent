...
//
// NOTE: playwright-handoff-task.json has no explicit `tags` field per test case — tags
...
test.describe("[CDP] Task index — load & welcome", { tag: ["@regression", "@full"] }, () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      await expect(taskPage.heading).toHaveText("What can I do for you?");
...
test.describe("[CDP] Task index — composer", { tag: ["@regression", "@full"] }, () => {
  test(
...
test.describe("[CDP] Task index — scenario cards", { tag: ["@regression", "@full"] }, () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      // No navigation, no auto-submit (source: handleCardSelect only calls fillText(),
      // never router.push / submit — see task.page.ts).
      await expect(page).toHaveURL(/\/task$/);
    },
  );
