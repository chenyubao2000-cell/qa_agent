...
test.describe("[CDP] Join Waitlist — button gating", { tag: ["@regression", "@full"] }, () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      // Gated only by isSubmitting, not by field completeness — stays enabled with an empty form.
...
test.describe("[CDP] Join Waitlist — submit gating", { tag: ["@regression", "@full"] }, () => {
  test(
    ...
    async ({ page, i18n }) => {
      ...
      // Safe: onClick short-circuits to the toast before any network call when email
      // is unverified — never exercises the real verified-submit path.
