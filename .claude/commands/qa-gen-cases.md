...
> **Note**: qa-gen-cases only creates the output directory, it does not generate test infrastructure files like playwright.config.ts or fixtures.ts.
> Those files are generated by subsequent `/qa-run-prd` or `/qa-explore` on first execution.
