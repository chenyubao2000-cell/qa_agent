...
> **Note**: qa-gen-cases only creates the output directory. It does NOT generate test infrastructure (playwright.config.ts, fixtures.ts, auth.setup.ts) or copy i18n message files.
> Those are generated by subsequent `/qa-run-prd` or `/qa-explore` on first execution. If you need i18n-aware case generation, the orchestrator reads `i18nMessagesDir` from projectContext but does NOT require local message copies — it uses them for key annotation only.
