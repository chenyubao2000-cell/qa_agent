...
> **通用能力**: 自动分析源码中的外部依赖（HTTP 调用、LLM 调用、DB 查询），生成三层 Mock 配置（L1 网络层 / L2 LLM 层 / L3 数据层）。
> 零侵入方式：不修改业务代码，通过网络拦截、LLM 替身、seed 数据实现测试隔离。
...
│  Eval：必须调用真实 LLM（eval 的价值所在）             │
├─────────────────────────────────────────────────────┤
│  L3 数据层 Mock                                      │
│  Drizzle Schema → seed 脚本 + fixture 管理            │
│  方式：从 Schema 自动生成 seed 数据                    │
│  隔离：worker-scope fixture，测试前 seed / 测试后清理  │
└─────────────────────────────────────────────────────┘
```
...
| 参数 | 必填 | 说明 | 示例 |
|------|:----:|------|------|
| `sourceDir` | YES | 源代码目录（扫描 import/fetch/axios 调用） | `/Users/x/project/src` |
| `schemaDir` | NO | Drizzle Schema 目录（L3 数据层 Mock） | `/Users/x/project/src/db/schema` |
...
**生成规则**：
- 从源码中提取 URL pattern + HTTP method
- 推断 response 类型（从 TypeScript 类型 / 实际调用上下文）
- 生成合理的 mock 数据（基于字段名推断）
- 外部第三方 API 生成固定 mock → 确保测试稳定性
- 内部 API 可选择 passthrough → 集成测试不 mock 内部调用

#### L2 — LLM Fixture 生成

**单元/集成测试：MockLanguageModelV2**
...
3. 测试运行时按 input 匹配 → 返回录制的 output
...
- 按字段名推断合理的 seed 数据
- 生成 `seed()` + `cleanup()` 函数对
- worker-scope 隔离：每个 worker 使用独立 seed 数据（ID 带 worker 前缀）
...
| MSW server setup | `tests/mocks/server.ts` | MSW server 初始化（Node / Browser） |
...
| Seed scripts | `tests/mocks/seeds/*.ts` | Drizzle seed + cleanup 脚本 |
| Mock 配置清单 | `tests/mocks/mock-manifest.json` | 所有 mock 依赖的索引 |

### mock-manifest.json 结构

```json
{
  "_generatedAt": "2026-04-16T00:00:00Z",
  "_sourceDir": "/Users/x/project/src",
  "l1_network": {
    "handlers": [
      {
...
| 集成测试 | MSW handler（内部 API passthrough） | MockLanguageModelV2 | Seed + cleanup |
...
1. 提供源代码目录 → Skill 自动扫描依赖
2. 审核 mock-manifest.json → 确认 mock 准确性（首次配置需人工审核）
3. 将 MSW server setup 集成到测试配置
4. 如需 LLM 录制回放 → 配置 Langfuse 项目名
5. 如需数据层 Mock → 提供 Drizzle Schema 目录
