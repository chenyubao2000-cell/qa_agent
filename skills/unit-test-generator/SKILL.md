...
> **通用能力**: 为 PR diff 中新增/修改的代码生成 Vitest/pytest 单元测试。
> 自动检测项目测试框架，参考已有测试风格，按等价类 + 边界值分析设计测试用例。
...
| `testStyle` | NO | 强制指定框架：`vitest` / `pytest` / `jest`，未指定则自动检测 |
...
3. Glob("$projectDir/**/pytest.ini") OR
   Glob("$projectDir/**/pyproject.toml") +
   Grep("pytest", "$projectDir/pyproject.toml")   → pytest
4. Glob("$projectDir/**/*.test.ts")               → Vitest (default for TS)
5. Glob("$projectDir/**/test_*.py")               → pytest (default for Python)
6. 无法检测 → 报错，要求用户指定 --style
```
...
提取：
- 函数名、导出方式（export function / export default / module.exports / class method）
- 参数列表：名称、类型（TS type annotation / Python type hint / JSDoc）
- 返回类型（显式注解或从 return 语句推断）
- 分支路径：if/else 数量、switch cases、try/catch、early return
...
#### 3.1 代码结构（Vitest/Jest）

```typescript
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
// 或 jest: import { describe, it, expect, jest } from '@jest/globals';
...
from app.services.target_module import target_function
...
class TestTargetFunction:
    ...
    @patch('app.services.target_module.external_api')
    async def test_resolves_with_data_on_success(self, mock_api):
        mock_api.get.return_value = mock_data
        result = await target_function(id)
...
If 存在 __tests__/ 目录:
  ...
  → 文件名: {sourceName}.test.{ext}

If 测试文件与源文件同级:
  → 同级创建 {sourceName}.test.{ext}

If 存在独立 tests/ 目录 (Python 项目常见):
  → 在 tests/ 下镜像源目录结构
  → 文件名: test_{sourceName}.py
...
| 依赖类型 | Mock 方式 (Vitest/Jest) | Mock 方式 (pytest) |
|----------|------------------------|-------------------|
| HTTP 请求 | MSW (`msw/node`) 或 `vi.mock('axios')` | `responses` 或 `unittest.mock.patch` |
| 数据库 | `vi.mock('../db')` + 返回 fixture 数据 | `pytest-mock` + fixture |
| 文件系统 | `memfs` 或 `vi.mock('fs')` | `tmp_path` fixture 或 `unittest.mock` |
| 环境变量 | `vi.stubEnv('KEY', 'value')` | `monkeypatch.setenv('KEY', 'value')` |
| 时间 | `vi.useFakeTimers()` | `freezegun` |
| 随机数 | `vi.spyOn(Math, 'random').mockReturnValue(0.5)` | `unittest.mock.patch('random.random')` |
| 内部函数 | **不 Mock** — 测试真实行为 | **不 Mock** |
| 第三方库 | `vi.mock('library')` | `unittest.mock.patch('module.library')` |
