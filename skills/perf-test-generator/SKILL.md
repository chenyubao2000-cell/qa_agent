...
> **通用能力**: 为 AI 应用生成 k6 性能测试脚本，支持 AI Streaming 专属指标（TTFT、Stream Throughput、WebSocket 重连等）。
...
export default function () {
  ...
  const res = ws.connect(url, {}, function (socket) {
    ...
    // Keep connection alive for test duration, then close
    socket.setTimeout(() => {
      socket.close();
    }, 30000);
  });
...
// Scenario 1: Browsing (REST API calls)
...
export function chatScenario() {
  ...
  sleep(Math.random() * 5 + 3); // 3-8s think time (reading AI response)
}
```
...
```json
{
  "_updatedAt": "2026-04-16T00:00:00Z",
  "_commit": "abc1234",
  "endpoints": {
    "/api/chat/stream": {
      "type": "sse",
...
4. 运行 `/qa-perf-test --target <endpoint>` → 自动生成 k6 脚本
